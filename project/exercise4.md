...
> **Tip:** Go to **CloudFormation → your stack → Outputs tab** to find exact bucket names and the `ReplicationRoleARN`.
...
5. Click on the rule to see details and confirm:
   ...
   - **Destination bucket:** `destination-bucket-{DeploymentID}` ✅
...
4. Click on the role and confirm:
   ...
   - **Tags:** `DeploymentID` tag matches your CloudLabs Deployment ID ✅
...
> You can also find the exact path in **CloudFormation → Outputs → `ReplicationTestObjectPath`**.

> This file was automatically uploaded by the Lambda function (`WebsiteFileUploaderFunction`) when the stack was deployed.
...
2. Go to **S3** → click on **`destination-bucket-{DeploymentID}`**
...
| `test-object.txt` exists in `source-bucket-{DeploymentID}` | ✅ |
| `test-object.txt` replicated to `destination-bucket-{DeploymentID}` | ✅ |
